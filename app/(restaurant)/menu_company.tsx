...
export default function RestaurantMenu() {
    ...
    const renderIngredientName = (ing: string | IngredientObject) => {
                            ...
                            {selectedPlate.sections?.map((sec, sIdx) => (
                                <View key={sec.id || sIdx} style={{ marginBottom: 25 }}>
                                    ...
                                    {sec.options?.map((opt, oIdx) => {
                                        const price = Number(opt.extraPrice);
                                        return (
                                            <View key={opt.id || oIdx} style={{ marginBottom: 8, paddingLeft: 10 }}>
                                                <Text style={styles.listBullet}>
                                                    • {opt.name} {price > 0 ? `(+₡${price.toLocaleString()})` : ''}
                                                </Text>
                            ...
                            <View style={styles.tagWrapper}>
                                {selectedPlate.baseIngredients?.map((ing, idx) => (
                                    <View key={idx} style={styles.tagChip}>
                                        <Text style={styles.tagText}>{renderIngredientName(ing)}</Text>
                                    </View>
                                ))}
                            </View>
